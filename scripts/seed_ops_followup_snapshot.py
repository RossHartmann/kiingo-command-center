#!/usr/bin/env python3
"""Seed: Ops Follow-Up Snapshot — daily pipeline follow-up health from CRM SharePoint."""
...
SLUG = "ops-followup-snapshot"
NAME = "Ops Follow-Up Snapshot"
SCREEN_ID = "ops-followup"

instructions = r"""Read today's OPS Follow-Up Metrics Report from the CRM SharePoint site and extract pipeline follow-up health data.
...
# ---------- Initial snapshot data (from Feb 18, 2026 OPS report) ----------

followup_ranges = [
    {"range": "0-3d", "healthy": 3, "warning": 0, "danger": 0},
    {"range": "3-7d", "healthy": 8, "warning": 0, "danger": 0},
    {"range": "7-14d", "healthy": 0, "warning": 5, "danger": 0},
    {"range": "14-30d", "healthy": 0, "warning": 9, "danger": 0},
    {"range": "30-60d", "healthy": 0, "warning": 0, "danger": 10},
    {"range": "60-90d", "healthy": 0, "warning": 0, "danger": 16},
    {"range": "90-180d", "healthy": 0, "warning": 0, "danger": 1},
    {"range": "180+d", "healthy": 0, "warning": 0, "danger": 6},
]

rep_breakdown = [
    {"rep": "kym", "totalDeals": 23, "active": 22, "likelyCold": 1, "coldPct": 4.3},
    {"rep": "ross", "totalDeals": 10, "active": 10, "likelyCold": 0, "coldPct": 0},
    {"rep": "josh", "totalDeals": 6, "active": 6, "likelyCold": 0, "coldPct": 0},
    {"rep": "david", "totalDeals": 4, "active": 3, "likelyCold": 1, "coldPct": 25.0},
    {"rep": "michael", "totalDeals": 4, "active": 4, "likelyCold": 0, "coldPct": 0},
    {"rep": "james", "totalDeals": 3, "active": 3, "likelyCold": 0, "coldPct": 0},
    {"rep": "jordan", "totalDeals": 2, "active": 2, "likelyCold": 0, "coldPct": 0},
...
def make_initial_html():
    html = template_jsx
    html = html.replace("TOTAL_OPEN_DEALS_PLACEHOLDER", "58")
    html = html.replace("LIKELY_COLD_PLACEHOLDER", "2")
    html = html.replace("OVERDUE_FOLLOWUPS_PLACEHOLDER", "37")
    html = html.replace("AVG_DAYS_PLACEHOLDER", "39.6")
...
METADATA = json.dumps({
    "aliases": ["ops follow-up", "ops followup", "operations follow-up", "ops pipeline health"],
})
    ...
    if cursor.fetchone()[0] > 0:
        ...
        cursor.execute(
            """INSERT INTO screen_metrics
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (binding_id, SCREEN_ID, metric_id, 0, "wide", 0, 0, 8, 14),
        )
