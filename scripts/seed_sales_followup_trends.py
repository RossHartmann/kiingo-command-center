#!/usr/bin/env python3
"""Seed: Sales Follow-Up Trends — multi-day trend tracking from CRM SharePoint."""
...
SLUG = "sales-followup-trends"
NAME = "Sales Follow-Up Trends"
SCREEN_ID = "dept-sales"

instructions = r"""Track daily trends in SALES follow-up pipeline health by reading the last 14 daily SALES Follow-Up Metrics Reports from the CRM SharePoint site.
...
daily_data = [
    {"date": "2/18", "totalDeals": 189, "likelyCold": 43, "overdue": 21, "avgDays": 9.4, "healthy": 126, "atRisk": 49},
]
...
METADATA = json.dumps({
    "aliases": ["sales follow-up trends", "follow-up trends", "pipeline trends", "staleness trends"],
})
    ...
    if cursor.fetchone()[0] > 0:
        ...
        cursor.execute(
            """INSERT INTO screen_metrics
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (binding_id, SCREEN_ID, metric_id, 3, "wide", 0, 38, 8, 14),
        )
